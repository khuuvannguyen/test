...
create table LICH  (
	...
	GHICHU nvarchar(50),
	CONSTRAINT FK_LICH_MATAIXEPHU FOREIGN KEY (MATAIXEPHU) REFERENCES TAIXE(MATAIXE),
	CONSTRAINT FK_LICH_MATAIXECHINH FOREIGN KEY (MATAIXECHINH) REFERENCES TAIXE(MATAIXE),
	CONSTRAINT FK_LICH_XE FOREIGN KEY (SOHIEUXE) REFERENCES XE(SOHIEUXE)
	)
	GO
/*TẠO TABLE VÉ (5)*/
create table VE  (
	...
	MACHUYENDI char(11),
	SDT char(10),
	CONSTRAINT VE_FK_LICH FOREIGN KEY (MACHUYENDI) REFERENCES LICH(MACHUYENDI),
	CONSTRAINT VE_FK_KHACH FOREIGN KEY (SDT) REFERENCES KHACH(SDT)
	)
	GO
...
create table CHOHANG  (
	...
	CONSTRAINT CH_FK_LICH FOREIGN KEY (MACHUYENDI) REFERENCES LICH(MACHUYENDI),
	)
	GO
...
BEGIN
	...
	IF @SOGHE < 10
		SET @MASOVE = '00' + CAST(@SOGHE AS VARCHAR(3)) + '.' + @MACHUYENDI
	ELSE
		SET @MASOVE = '0' + CAST(@SOGHE AS VARCHAR(3)) + '.' + @MACHUYENDI
	IF NOT EXISTS 
	(
		...
		BEGIN
			...
			(
				...
				BEGIN
					IF( @SOGHE > (SELECT XE.SOGHE FROM XE, LICH WHERE LICH.MACHUYENDI = @MACHUYENDI AND LICH.SOHIEUXE = XE.SOHIEUXE)
						OR @SOGHE <= 0)
						BEGIN
							PRINT N'SỐ GHẾ KHÔNG HỢP LỆ'
						END
					ELSE
						BEGIN
							SET NOCOUNT ON
							...
							SET NOCOUNT ON
							PRINT N'THÊM THÀNH CÔNG VÉ ' + @MASOVE + N' VÀO CHUYẾN ' + @MACHUYENDI
						END
				END
			ELSE 
				PRINT N'KHÔNG TỒN TẠI CHUYẾN ' + @MACHUYENDI
